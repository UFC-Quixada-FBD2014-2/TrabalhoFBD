...
$(document).ready(function () {
	...
	function initialize() {
		...
		map = new google.maps.Map(document.getElementById("mapaVisitados"), options);
	...
	function carregarPontosTuristicos(){
		$.ajax({
			url:"../fronteiras/ajax/buscarTodosOsPontosTuristicosVisitados.php",
			dataType:"json",
		...
		geocoder.geocode({ 'address': endereco + ', Brasil', 'region': 'BR' }, function (results, status) {
			if (status == google.maps.GeocoderStatus.OK) {
				if (results[0]) {
					...
					$('#txtEnderecoVisitados').val(results[0].formatted_address);
	...
	$("#btnEnderecoVisitados").click(function() {
		if($(this).val() != "")
			carregarNoMapa($("#txtEnderecoVisitados").val());
	})
	
	$("#txtEnderecoVisitados").blur(function() {
		if($(this).val() != "")
			carregarNoMapa($(this).val());
	});
	
	$("#txtEnderecoVisitados").autocomplete({
